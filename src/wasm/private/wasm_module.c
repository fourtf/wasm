...
// param_count) {
//   type->param_types = wasm_alloc_array(enum wasm_valtype, param_count);
//   type->param_types_end = type->param_types + param_count;
// }
  ...
  while (wasm_read(reader, &section_type, 1)) {
    ...
    case 1: {
      ...
      for (size_t i = 0; i < type_count; i++) {
        ...
        wasm_reader_assert(reader, wasm_read_byte(reader) == 0x60);
...
wasm_module *wasm_load_module_from_file(const char *file_name) {
  ...
  // Wrap file in reader.
  wasm_reader reader;
  wasm_init_file_device(&reader, file);
